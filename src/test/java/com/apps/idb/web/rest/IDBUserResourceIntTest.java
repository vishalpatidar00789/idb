...
public class IDBUserResourceIntTest {
    ...
    private static final String DEFAULT_SESSION_TOKEN = "AAAAAAAAAA";
    private static final String UPDATED_SESSION_TOKEN = "BBBBBBBBBB";
    ...
    private static final LocalDate DEFAULT_LAST_DEACTIVATED_DATE = LocalDate.ofEpochDay(0L);
    private static final LocalDate UPDATED_LAST_DEACTIVATED_DATE = LocalDate.now(ZoneId.systemDefault());

    private static final UserRoles DEFAULT_USER_ROLES = UserRoles.ADMIN;
    private static final UserRoles UPDATED_USER_ROLES = UserRoles.USER;
    ...
    private static final Boolean DEFAULT_IS_REPORTED_SCAM = false;
    private static final Boolean UPDATED_IS_REPORTED_SCAM = true;

    private static final LocalDate DEFAULT_LAST_LOGOUT = LocalDate.ofEpochDay(0L);
    private static final LocalDate UPDATED_LAST_LOGOUT = LocalDate.now(ZoneId.systemDefault());

    private static final LocalDate DEFAULT_LAST_ACTIVATED_DATE = LocalDate.ofEpochDay(0L);
    private static final LocalDate UPDATED_LAST_ACTIVATED_DATE = LocalDate.now(ZoneId.systemDefault());
    ...
    @Autowired
    private IDBUserRepository iDBUserRepository;

    @Autowired
    private IDBUserMapper iDBUserMapper;

    @Autowired
    private IDBUserService iDBUserService;
    ...
    public void setup() {
        ...
        final IDBUserResource iDBUserResource = new IDBUserResource(iDBUserService);
        this.restIDBUserMockMvc = MockMvcBuilders.standaloneSetup(iDBUserResource)
     ...
     */
    public static IDBUser createEntity(EntityManager em) {
        IDBUser iDBUser = new IDBUser()
            ...
            .isReportedScam(DEFAULT_IS_REPORTED_SCAM)
            .lastLogout(DEFAULT_LAST_LOGOUT)
    ...
    public void initTest() {
        iDBUser = createEntity(em);
    }

    @Test
    @Transactional
    public void createIDBUser() throws Exception {
        int databaseSizeBeforeCreate = iDBUserRepository.findAll().size();

        // Create the IDBUser
        IDBUserDTO iDBUserDTO = iDBUserMapper.toDto(iDBUser);
        restIDBUserMockMvc.perform(post("/api/idb-users")
            .contentType(TestUtil.APPLICATION_JSON_UTF8)
            .content(TestUtil.convertObjectToJsonBytes(iDBUserDTO)))
            .andExpect(status().isCreated());

        // Validate the IDBUser in the database
        List<IDBUser> iDBUserList = iDBUserRepository.findAll();
        assertThat(iDBUserList).hasSize(databaseSizeBeforeCreate + 1);
        IDBUser testIDBUser = iDBUserList.get(iDBUserList.size() - 1);
        ...
        assertThat(testIDBUser.getLastDeactivatedDate()).isEqualTo(DEFAULT_LAST_DEACTIVATED_DATE);
        ...
        assertThat(testIDBUser.getVerificationMethod()).isEqualTo(DEFAULT_VERIFICATION_METHOD);
        ...
        assertThat(testIDBUser.getLastActivatedDate()).isEqualTo(DEFAULT_LAST_ACTIVATED_DATE);
        ...
        assertThat(testIDBUser.getLastUpdatedDate()).isEqualTo(DEFAULT_LAST_UPDATED_DATE);
        assertThat(testIDBUser.getLastUpdatedBy()).isEqualTo(DEFAULT_LAST_UPDATED_BY);
    }

    @Test
    @Transactional
    public void createIDBUserWithExistingId() throws Exception {
        int databaseSizeBeforeCreate = iDBUserRepository.findAll().size();

        // Create the IDBUser with an existing ID
        iDBUser.setId(1L);
        IDBUserDTO iDBUserDTO = iDBUserMapper.toDto(iDBUser);
    ...
    public void checkEmailIsRequired() throws Exception {
        int databaseSizeBeforeTest = iDBUserRepository.findAll().size();
        // set the field null
        iDBUser.setEmail(null);

        // Create the IDBUser, which fails.
        IDBUserDTO iDBUserDTO = iDBUserMapper.toDto(iDBUser);
    ...
    @Test
    @Transactional
    public void checkUserRolesIsRequired() throws Exception {
        int databaseSizeBeforeTest = iDBUserRepository.findAll().size();
        // set the field null
        iDBUser.setUserRoles(null);
    ...
    @Test
    @Transactional
    public void getAllIDBUsers() throws Exception {
        // Initialize the database
        iDBUserRepository.saveAndFlush(iDBUser);
        ...
        restIDBUserMockMvc.perform(get("/api/idb-users?sort=id,desc"))
            ...
            .andExpect(jsonPath("$.[*].sessionToken").value(hasItem(DEFAULT_SESSION_TOKEN.toString())))
            .andExpect(jsonPath("$.[*].lastLoginDate").value(hasItem(DEFAULT_LAST_LOGIN_DATE.toString())))
            .andExpect(jsonPath("$.[*].lastDeactivatedDate").value(hasItem(DEFAULT_LAST_DEACTIVATED_DATE.toString())))
            .andExpect(jsonPath("$.[*].userRoles").value(hasItem(DEFAULT_USER_ROLES.toString())))
            .andExpect(jsonPath("$.[*].verified").value(hasItem(DEFAULT_VERIFIED.booleanValue())))
            .andExpect(jsonPath("$.[*].verificationMethod").value(hasItem(DEFAULT_VERIFICATION_METHOD.toString())))
            .andExpect(jsonPath("$.[*].isReportedScam").value(hasItem(DEFAULT_IS_REPORTED_SCAM.booleanValue())))
            .andExpect(jsonPath("$.[*].lastLogout").value(hasItem(DEFAULT_LAST_LOGOUT.toString())))
            .andExpect(jsonPath("$.[*].lastActivatedDate").value(hasItem(DEFAULT_LAST_ACTIVATED_DATE.toString())))
    ...
    @Test
    @Transactional
    public void getIDBUser() throws Exception {
        // Initialize the database
        iDBUserRepository.saveAndFlush(iDBUser);

        // Get the iDBUser
        restIDBUserMockMvc.perform(get("/api/idb-users/{id}", iDBUser.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            ...
            .andExpect(jsonPath("$.lastDeactivatedDate").value(DEFAULT_LAST_DEACTIVATED_DATE.toString()))
            ...
            .andExpect(jsonPath("$.verificationMethod").value(DEFAULT_VERIFICATION_METHOD.toString()))
            .andExpect(jsonPath("$.isReportedScam").value(DEFAULT_IS_REPORTED_SCAM.booleanValue()))
            .andExpect(jsonPath("$.lastLogout").value(DEFAULT_LAST_LOGOUT.toString()))
            .andExpect(jsonPath("$.lastActivatedDate").value(DEFAULT_LAST_ACTIVATED_DATE.toString()))
    ...
    @Test
    @Transactional
    public void getNonExistingIDBUser() throws Exception {
        // Get the iDBUser
        restIDBUserMockMvc.perform(get("/api/idb-users/{id}", Long.MAX_VALUE))
            .andExpect(status().isNotFound());
    }

    @Test
    @Transactional
    public void updateIDBUser() throws Exception {
        // Initialize the database
        iDBUserRepository.saveAndFlush(iDBUser);
        ...
        // Update the iDBUser
        IDBUser updatedIDBUser = iDBUserRepository.findById(iDBUser.getId()).get();
        // Disconnect from session so that the updates on updatedIDBUser are not directly saved in db
        em.detach(updatedIDBUser);
        updatedIDBUser
            .email(UPDATED_EMAIL)
            .password(UPDATED_PASSWORD)
            .activated(UPDATED_ACTIVATED)
            .accessToken(UPDATED_ACCESS_TOKEN)
            .sessionToken(UPDATED_SESSION_TOKEN)
            ...
            .isReportedScam(UPDATED_IS_REPORTED_SCAM)
            .lastLogout(UPDATED_LAST_LOGOUT)
            .lastActivatedDate(UPDATED_LAST_ACTIVATED_DATE)
            .createdDate(UPDATED_CREATED_DATE)
            .createdBy(UPDATED_CREATED_BY)
            .lastUpdatedDate(UPDATED_LAST_UPDATED_DATE)
            .lastUpdatedBy(UPDATED_LAST_UPDATED_BY);
        IDBUserDTO iDBUserDTO = iDBUserMapper.toDto(updatedIDBUser);
        ...
        assertThat(testIDBUser.getLastDeactivatedDate()).isEqualTo(UPDATED_LAST_DEACTIVATED_DATE);
        ...
        assertThat(testIDBUser.getVerificationMethod()).isEqualTo(UPDATED_VERIFICATION_METHOD);
        ...
        assertThat(testIDBUser.getLastActivatedDate()).isEqualTo(UPDATED_LAST_ACTIVATED_DATE);
        ...
        assertThat(testIDBUser.getLastUpdatedDate()).isEqualTo(UPDATED_LAST_UPDATED_DATE);
        assertThat(testIDBUser.getLastUpdatedBy()).isEqualTo(UPDATED_LAST_UPDATED_BY);
    }

    @Test
    @Transactional
    public void updateNonExistingIDBUser() throws Exception {
        int databaseSizeBeforeUpdate = iDBUserRepository.findAll().size();
    ...
    @Test
    @Transactional
    public void deleteIDBUser() throws Exception {
        // Initialize the database
        iDBUserRepository.saveAndFlush(iDBUser);
    ...
    public void equalsVerifier() throws Exception {
        TestUtil.equalsVerifier(IDBUser.class);
        IDBUser iDBUser1 = new IDBUser();
        iDBUser1.setId(1L);
        IDBUser iDBUser2 = new IDBUser();
        iDBUser2.setId(iDBUser1.getId());
        assertThat(iDBUser1).isEqualTo(iDBUser2);
        iDBUser2.setId(2L);
        assertThat(iDBUser1).isNotEqualTo(iDBUser2);
        iDBUser1.setId(null);
        assertThat(iDBUser1).isNotEqualTo(iDBUser2);
    }
    ...
    public void dtoEqualsVerifier() throws Exception {
        TestUtil.equalsVerifier(IDBUserDTO.class);
        IDBUserDTO iDBUserDTO1 = new IDBUserDTO();
        iDBUserDTO1.setId(1L);
        IDBUserDTO iDBUserDTO2 = new IDBUserDTO();
        assertThat(iDBUserDTO1).isNotEqualTo(iDBUserDTO2);
        iDBUserDTO2.setId(iDBUserDTO1.getId());
        assertThat(iDBUserDTO1).isEqualTo(iDBUserDTO2);
        iDBUserDTO2.setId(2L);
        assertThat(iDBUserDTO1).isNotEqualTo(iDBUserDTO2);
        iDBUserDTO1.setId(null);
        assertThat(iDBUserDTO1).isNotEqualTo(iDBUserDTO2);
    }
